...
const properties = [
  {
    ...
    features: JSON.stringify(["Ocean View", "Balcony", "Pool", "Gym", "Security", "Parking"]),
    yearBuilt: 2019,
  ...
  },
  {
    ...
    features: JSON.stringify(["Fireplace", "Backyard", "Garage", "Updated Kitchen", "Master Suite", "Basement"]),
    yearBuilt: 2010,
    agent: JSON.stringify({
      name: "Robert Obi",
      phone: "(081) 987-6543",
  ...
  },
  {
    ...
    features: JSON.stringify(["City Views", "Smart Home", "Concierge", "Rooftop Pool", "Private Theater", "Wine Cellar"]),
    yearBuilt: 2020,
    agent: JSON.stringify({
  ...
  },
  {
    ...
    features: JSON.stringify(["Garden", "Fireplace", "Detached Studio", "Updated Kitchen", "Original Details", "Porch"]),
    yearBuilt: 1985,
    agent: JSON.stringify({
      name: "Emma Wilson",
      phone: "(090) 234-5678",
  ...
  },
  {
    ...
    features: JSON.stringify(["Rooftop Deck", "Energy Efficient", "Two-Car Garage", "Open Floor Plan", "City Views", "Smart Home"]),
    yearBuilt: 2021,
    agent: JSON.stringify({
      name: "David Nwosu",
      phone: "(080) 345-6789",
  ...
  },
  {
    ...
    features: JSON.stringify(["Waterfront", "Mountain Views", "Fireplace", "Deck", "2 Acres", "Updated Kitchen"]),
    yearBuilt: 2005,
    agent: JSON.stringify({
      name: "Sarah Thompson",
      phone: "(070) 876-5432",
...
async function setupDatabase() {
  ...
  try {
    ...
    if (rows[0].count === 0) {
      console.log('Inserting sample properties data...');
      
      // Insert sample properties
      for (const property of properties) {
        await connection.query(
          'INSERT INTO properties (title, price, location, description, bedrooms, bathrooms, area, type, isNew, imageUrl, images, features, yearBuilt, agent) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [
            property.title,
            property.price,
            property.location,
      ...
      console.log('Properties table already has data, skipping sample data insertion');
    }

    // Also create a backup JSON file for fallback
    const dataDir = path.join(__dirname, '..', 'data');
    await fs.mkdir(dataDir, { recursive: true });
    
    const [propertiesData] = await connection.query('SELECT * FROM properties');
    const jsonProperties = propertiesData.map(p => ({
      ...p,
    ...
    await fs.writeFile(
      ...
      JSON.stringify(jsonProperties, null, 2)
    );
    
    console.log('Backup JSON file created successfully');
